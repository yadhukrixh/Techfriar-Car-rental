  ...
  const handleSubmit = async (values: BillingFormValues) => {
    const returnLocation =
      returnLocationType === 'custom'
        ? customReturnLocation?.toString()
        : returnLocationType === 'delivery'
        ? location?.toString()
        : 'company';
    ...
    <Form className={styles.billingForm} onFinish={handleSubmit} layout="vertical">
      <div className={styles.section}>
        ...
        <div className={styles.readOnlyField}>
          ...
          <span>
            {dates && dates.length > 0
              ? `${dates[0]?.toISOString().split('T')[0]} to ${
        ...
        <button
          ...
          type="button"
        >
          <i className="ri-eye-line"></i> Delivery Location
        </button>
      </div>
      ...
      <div className={styles.section}>
            ...
            { pattern: /^\d{10}$/, message: 'Mobile number should be exactly 10 digits.' },
            {
              validator(_, value) {
                ...
                return Promise.reject(
                  new Error('Secondary mobile number cannot be the same as primary mobile number.')
                );
              },
            },
          ]}
        ...
        <div className={styles.returnLocationSection}>
          ...
          <Form.Item>
            <Radio.Group value={returnLocationType} onChange={(e) => handleLocationSelect(e.target.value)}>
            ...
            {customReturnLocation && (
              <button
                ...
                type="button"
              >
                <i className="ri-edit-2-line"></i> Return Location
              </button>
            )}
          </Form.Item>
        </div>
